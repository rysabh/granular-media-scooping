"""
High-level scooping pipeline (boundary-centered version):

Video
  -> OWLv2 open-vocabulary detection
  -> object/task-centric crops
  -> DINOv2 object-crop embeddings + DINOv2 global-frame embeddings + relative spoon-location deltas
  -> aggregate object-centered + scene-level + spatial temporal signal Z(t)
  -> optional PCA + embedding deltas
...
EMBEDDING_DEBUG_PATH = "dino_object_global_spatial_embeddings.npz"
...
# Add relative spatial deltas so PELT can see spoon movement relative
# to the source and target containers without manual skill rules.
USE_RELATIVE_SPATIAL_DELTAS = True

# Embedding signal options.
PCA_DIMS = 64
...
Required fields:
- spoon_occupancy:
    empty / contains_material / unclear

- spoon_to_source:
    inside / touching / near / above / far / unclear

- spoon_to_target:
    inside / touching / near / above / far / unclear

- sugar_pile:
    intact / disturbed / reduced / unclear

- source_container:
    visible / partially_visible / not_visible / unclear

- target_container:
    empty / receiving_material / contains_material / visible / partially_visible / not_visible / unclear
...
Good:
"spoon_occupancy=contains_material; spoon_to_source=inside; target_container=empty"
...
def extract_object_crop_embeddings(
    ...
    """Return aggregate sequence Z[t] and per-object/global embedding streams.

    The rupture signal combines:
      1. object-centered DINO embeddings from OWLv2 crops
      2. optional global-frame DINO embedding from the full scene
      3. optional relative spatial deltas:
            delta_spoon_source = center(spoon) - center(source_container)
            delta_spoon_target = center(spoon) - center(target_container)

    Object crops preserve object appearance/content.
    Global-frame embeddings preserve scene layout and rough object location.
    Relative deltas make spoon movement with respect to source/target explicit.
    """
    dino_model, dino_processor, dino_device = load_dino()
    cap = cv2.VideoCapture(video_path)
    object_names = list(OWL_OBJECT_PROMPTS.keys())

    streams: Dict[str, List[np.ndarray]] = {name: [] for name in object_names}
    if USE_GLOBAL_FRAME_EMBEDDING:
        streams["global_frame"] = []
    if USE_RELATIVE_SPATIAL_DELTAS:
        streams["delta_spoon_source"] = []
        streams["delta_spoon_target"] = []
    ...
    for idx, (frame_id, det) in enumerate(zip(frame_ids, detections)):
        ...
        for name in OWL_OBJECT_PROMPTS.keys():
            crop = padded_crop(image, det[name].box)
            crops.append(crop)
        ...
        if USE_RELATIVE_SPATIAL_DELTAS:
            ...
            streams["delta_spoon_source"].append(
                relative_center_delta(
                    det["spoon"].box,
                    det["source_container"].box,
                    image_size,
                )
            )
            streams["delta_spoon_target"].append(
                relative_center_delta(
                    det["spoon"].box,
                    det["target_container"].box,
                    image_size,
                )
            )
    ...
    stream_names = [name for name in object_names if name in stream_arrays]
    if USE_GLOBAL_FRAME_EMBEDDING and "global_frame" in stream_arrays:
        stream_names.append("global_frame")
    if USE_RELATIVE_SPATIAL_DELTAS:
        for spatial_name in ["delta_spoon_source", "delta_spoon_target"]:
            if spatial_name in stream_arrays:
                stream_names.append(spatial_name)
    
    
    # Make spatial deltas matter more for rupture.
    SPATIAL_FEATURE_WEIGHT = 20.0
    ...
    if USE_RELATIVE_SPATIAL_DELTAS:
        ...
        "delta_spoon_target"
            ...
            if spatial_name in stream_arrays:
                stream_arrays[spatial_name] = (stream_arrays[spatial_name]* SPATIAL_FEATURE_WEIGHT
            )
...
def prepare_signal_for_ruptures(Z: np.ndarray) -> Tuple[np.ndarray, Dict]:
    ...
    Z is the concatenated object + scene embedding vector:
        Z(t) = [z_spoon_crop, z_sugar_crop, z_source_crop, z_target_crop, z_global_scene, delta_spoon_source, delta_spoon_target]

    RBF inside PELT detects nonlinear similarity/distribution changes in this
    joint object-centered, scene-level, and relative-delta representation
    """
    ...
    info = {
        ...
        "interpretation": "joint object-centered + global-scene DINO embedding signal plus relative spoon-source/spoon-target deltas; PELT+RBF detects distribution/similarity shifts",
    }
...
def detect_change_points(video_path: str, total_frames: int, save_crops: bool = False) -> Tuple[List[int], Dict]:
    ...
    debug = {
        "pipeline": "OWLv2 detection -> object/task-centric crops + global frame + relative spoon-location deltas -> DINOv2/features -> aggregate -> PELT(model='rbf')",
        ...
        "uses_relative_spatial_deltas": USE_RELATIVE_SPATIAL_DELTAS,
        "spatial_delta_features": ["delta_spoon_source", "delta_spoon_target"],
    }
    with open(BOUNDARY_DEBUG_PATH, "w") as f:
        json.dump(debug, f, indent=2)

    print(f"Detected {len(internal_boundaries)} internal boundaries using OWLv2 + DINOv2 object/global embeddings + PELT(RBF).")
...
Use these controlled state variables:
...
- If the object is hard to see because it is occluded or out of frame, use unclear instead of guessing.
...
{{
  ...
  "state_variables": {{
    "spoon_occupancy": "empty/contains_material/unclear",
    "spoon_to_source": "inside/touching/near/above/far/unclear",
    "spoon_to_target": "inside/touching/near/above/far/unclear",
    "sugar_pile": "intact/disturbed/reduced/unclear",
    "source_container": "visible/partially_visible/not_visible/unclear",
    "target_container": "empty/receiving_material/contains_material/visible/partially_visible/not_visible/unclear"
  }},
  "visible_conditions": [
...
Ignore source_container visibility changes when choosing the skill.
...
approach:
PRE spoon_to_source is near or above
POST spoon_to_source is inside
spoon_occupancy does not change from contains_material to another value

scoop:
PRE spoon_occupancy is empty or unclear
POST spoon_occupancy is contains_material
Optional support: sugar_pile changes from intact to disturbed/reduced
FORBIDDEN if PRE spoon_occupancy is contains_material

lift:
PRE spoon_occupancy is contains_material AND PRE spoon_to_source is inside or touching
POST spoon_occupancy is contains_material AND POST spoon_to_source is above or near
FORBIDDEN if PRE spoon_to_source and POST spoon_to_source are the same

transport:
PRE spoon_occupancy is contains_material
POST spoon_occupancy is contains_material
AND spoon_to_target becomes closer to target
Typical change: far/unclear -> near/above/inside

dump:
PRE spoon_occupancy is contains_material
AND (
  POST spoon_occupancy is empty or unclear
  OR POST target_container is receiving_material or contains_material
)
FORBIDDEN if spoon_occupancy remains contains_material AND target_container does not change to receiving_material/contains_material

return:
PRE spoon_occupancy is empty or unclear AND PRE spoon_to_target is near/above/inside
POST spoon_occupancy is empty or unclear AND POST spoon_to_source is near/above/inside

none:
Choose none if:
- delta is empty
- only source_container visibility changed
...
def validate_skill_against_state_variables(
    ...
    if skill == "dump" and post_occ == "contains_material" and post_target not in {"receiving_material", "contains_material"}:
        return "none", "corrected_to_none: dump forbidden because spoon still contains material and target did not receive material"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build rupture-segment high-level graph using OWLv2 + DINOv2 + PELT(RBF) + VLM."
    )
...
def main() -> None:
    ...
    graph = {
        ...
        "pipeline": [
            ...
            "relative spoon-source and spoon-target spatial deltas",
            "DINOv2 object-crop and global-frame embedding extraction",
            "object-centered + scene-level + relative-delta feature aggregation",
            "optional PCA and embedding delta construction",
        ...
        "segmentation_method": "PELT_RBF_on_joint_DINOv2_object_crop_global_frame_and_relative_spatial_deltas",
        ...
        "embedding_model": {
            ...
            "object_streams": list(OWL_OBJECT_PROMPTS.keys()),
            "uses_global_frame_embedding": USE_GLOBAL_FRAME_EMBEDDING,
            "uses_relative_spatial_deltas": USE_RELATIVE_SPATIAL_DELTAS,
            "spatial_delta_features": ["delta_spoon_source", "delta_spoon_target"],
            "aggregation": "concatenate object crop embeddings, global-frame embedding, and relative spoon-source/spoon-target deltas per sampled frame",
        ...
        "change_point_detector": {
            ...
            "input_features": "joint DINOv2 embeddings from OWLv2 object crops plus optional global-frame embedding plus relative spoon-source/spoon-target spatial deltas, optionally PCA-reduced and concatenated with temporal embedding deltas",
